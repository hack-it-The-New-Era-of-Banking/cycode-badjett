...
const insight_post = catchAsync(async (req, res, next) => {
  ...
  const incomeSummary = Array.isArray(user.income)
    ? user.income.reduce((summary, income) => {
    ...
    input: [
      {
        ...
        content: `
            ...
            - Monthly Income: ${
              Array.isArray(user.income)
                ? user.income.reduce(
                    (sum, inc) =>
                      sum + inc.earnedIncome + inc.additionalCompensation,
                    0
                  )
                : 0
            }
